...
ENSEMBLE_CONFIG = {
    # ── 模型路径 ──
    'transformer_model': str(PROJECT_ROOT / 'model' / '60_158+39' / 'best_model.pth'),
    'transformer_scaler': str(PROJECT_ROOT / 'model' / '60_158+39' / 'scaler.pkl'),
    'lgbm_model': str(PROJECT_ROOT / 'LGBM' / 'model' / 'lgbm_model.pkl'),

    # ── 数据路径 ──
    'stock_data': str(PROJECT_ROOT / 'data' / 'stock_data.csv'),
    'test_csv': str(PROJECT_ROOT / 'data' / 'test.csv'),

    # ── MD-SRP 路径（仅 use_mdrp=True 时需要） ──
    'industry_csv': str(PROJECT_ROOT / 'data' / 'stock_industry.csv'),
    'index_csv': str(PROJECT_ROOT / 'data' / 'index_data.csv'),
    'use_mdrp': False,          # 默认关闭，需要时设为 True
    ...
    # ── 输出 ──
    'output_dir': str(PROJECT_ROOT / 'ensemble' / 'output'),
}
