...
from featurework import engineer_features_158plus39, create_ranking_dataset_vectorized
...
def _preprocess_common(df, stockid2idx, desc, drop_small_open=True, features=FEATURE_COLUMNS_158PLUS39):
    ...
    with mp.Pool(processes=num_processes) as pool:
        processed_list = list(tqdm(pool.imap(engineer_features_158plus39, groups), total=len(groups), desc=desc))
...
def collate_fn(batch):
    ...
    for i, (seq, tgt, rel, stock_idx) in enumerate(zip(sequences, targets, relevance, stock_indices)):
        ...
        if num_stocks < max_stocks:
            ...
            rel_pad = torch.zeros(pad_size, dtype=torch.float)
    ...
    if has_time_weight:
        result['time_weights'] = torch.stack(time_weights)
    if has_prior_bias:
        result['prior_biases'] = torch.stack(prior_biases)
    if has_market_state:
        result['market_states'] = torch.stack(market_states)
...
def main():
    ...
    print("创建市场标准化器...")
    market_normalizer = MarketNormalizer(thresholds=config.get('market_normalizer_thresholds', (-0.01, 0.01)))
    fit_data = train_data.copy()
    fit_data['datetime'] = pd.to_datetime(fit_data['日期'])
    market_normalizer.fit(fit_data[['datetime', 'label']])
    market_normalizer_path = os.path.join(output_dir, 'market_normalizer.pkl')
    joblib.dump(market_normalizer, market_normalizer_path)
