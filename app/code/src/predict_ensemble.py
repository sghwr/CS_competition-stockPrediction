...
def build_inference_sequences(data, features, sequence_length, stock_codes, latest_date):
...
def main():
    ...
    lgbm_model_path = os.path.join(config['output_dir'], 'lgbm_model.pkl')
    ...
    sequences_np, sequence_stock_codes = build_inference_sequences(
        processed, FEATURE_COLUMNS, sequence_length, stock_codes, latest_date
    )
    ...
    model = StockTransformer(input_dim=len(FEATURE_COLUMNS), config=config, num_stocks=len(stock_codes))
    ...
    if config.get('use_mdrp', False):
        ...
        state_extractor = MarketStateExtractor(index_csv=os.path.join(config['data_path'], 'index_data.csv'))
        onehot = state_extractor.get_state_onehot(date_str)
        vol = state_extractor.get_volatility(date_str)
        market_state = torch.from_numpy(np.concatenate([onehot, [vol]]).astype(np.float32)).float().to(device).unsqueeze(0)
    ...
    with torch.no_grad():
        ...
        transformer_scores = model(x, prior_bias=prior_bias, market_state=market_state).squeeze(0).cpu().numpy()
    
    X_pred = sequences_np[:, -1, :].astype(np.float32)
    lgbm_scores = lgbm_model.predict(X_pred)
    
    lgbm_score_map = dict(zip(sequence_stock_codes, lgbm_scores))
    tf_score_map = dict(zip(sequence_stock_codes, transformer_scores))
    
    common_stocks = sorted(set(sequence_stock_codes))
    tf_arr = np.array([tf_score_map[s] for s in common_stocks], dtype=np.float64)
    lgbm_arr = np.array([lgbm_score_map[s] for s in common_stocks], dtype=np.float64)
    ...
    w_tf = 0.6
    w_lgbm = 0.4
    ensemble_scores = w_tf * tf_norm + w_lgbm * lgbm_norm
    
    order = np.argsort(ensemble_scores)[::-1]
    ranked_stocks = [common_stocks[i] for i in order]
    
    top_k = 5
    if len(ranked_stocks) < top_k:
        raise ValueError(f'可预测股票不足{top_k}只')
    top5_ids = ranked_stocks[:top_k]
    top5_weights = np.array([0.2] * top_k)
    
    output_df = pd.DataFrame({'stock_id': top5_ids, 'weight': top5_weights})
    output_df.to_csv(output_path, index=False)
    
    print(f'预测日期: {latest_date.date()}')
    print(f'参与排序股票数: {len(common_stocks)}')
    print(f'Transformer权重: {w_tf}, LGBM权重: {w_lgbm}')
